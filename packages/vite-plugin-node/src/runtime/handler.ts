...
import type { NextFunction, ViteNodeHttpResponse, ViteNodeOptions } from './types.js'

const dirname_ = dirname(fileURLToPath(import.meta.url))

function createHandler<PlatformRequest>(options: ViteNodeOptions<PlatformRequest> = {}) {
...
async function writeHttpResponse(httpResponse: ViteNodeHttpResponse | RenderAssetHttpResponse, res: ServerResponse) {
  assert(httpResponse)
